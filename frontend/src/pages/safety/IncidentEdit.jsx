...
const IncidentEdit = () => {
  ...
  useEffect(() => {
    // Fetch sites and incident data
    const fetchData = async () => {
      try {
        const [sitesRes, incidentRes] = await Promise.all([
          axiosInstance.get("/api/sites", {
            headers: { Authorization: `Bearer ${user.token}` },
          }),
          axiosInstance.get(`/api/safety/${id}`, {
            headers: { Authorization: `Bearer ${user.token}` },
          }),
        ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await axiosInstance.put(`/api/safety/${id}`, formData, {
        headers: { Authorization: `Bearer ${user.token}` },
      });
  ...
  if (loadingData) {
    return <div className="text-center mt-10">Loading incident data...</div>;
  }
